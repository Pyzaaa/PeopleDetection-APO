...
def detect_image_single_run(img_path, model, score_thr, nms_thr):
    ...
    # NMS z parametrem nms_thr
    boxes, scores = nms(boxes, scores, nms_thr)

    return len(boxes), boxes, scores

def grid_search(img_path, model="hog_6x6-3x3-12_SGD-v2.joblib"):
    score_values = [2.5, 3, 3.5, 4, 4.5, 5]
    nms_values = [0.01, 0.1, 0.15, 0.2, 0.3, 0.4]

    results = []

    for score_thr in score_values:
        for nms_thr in nms_values:
            count, boxes, scores = detect_image_single_run(
                img_path, model, score_thr, nms_thr
            )
            ...
            print(f"score={score_thr:.2f}, nms={nms_thr:.2f} → wykrycia: {count}")

    return results

if __name__=="__main__":
    img_path = "data/valid/images/group-of-people-in-a-meeting-1367272-2_jpg.rf.9a2d1e6086b3691f11d52167cad402fa.jpg"
    results = grid_search(img_path)
